...
class MainDashboard(ctk.CTk):
    def __init__(self):
        ...
        # Keep exactly one CTk/Tk root. The splash is a non-blocking Toplevel.
        self.withdraw() # <--- Keep the main window hidden initially

        # Application State
        self.current_theme = "dark"
        self.theme = THEMES[self.current_theme]
        self.splash = None # <--- Initialize splash attribute
        ...
        # Build the main UI before showing the root so it never appears blank.
        self._create_ui()
        self.update_idletasks()

        # <--- DO NOT create the splash screen or call mainloop here!
        # <--- The __init__ method must finish quickly.

    def start(self): # <--- NEW METHOD TO START THE APPLICATION LIFECYCLE
        """Show splash screen and start the main event loop."""
        self.splash = SplashScreen(self, on_complete=self._show_main_window)
        self.splash.lift()
        self.splash.focus_force()
        self.mainloop() # <--- The mainloop is now called from here
    ...
    def _create_module_card(self, parent, module_data):
        ...
        def open_module(e=None):
            module_data["command"]()

        def bind_card_events(widget):
        ...
        launch_btn.configure(command=open_module)
...
© 2024-2025 All Rights Reserved"""
    ...
    def launch_properties_app(self):
        self._run_module("src.gui.fluid_properties_gui")

    def launch_statics_app(self):
        self._run_module("src.gui.fluid_statics_gui")
        
    def launch_dynamics_app(self):
        self._run_module("src.gui.fluid_dynamics_gui")

    def launch_dimensional_app(self):
        self._run_module("src.gui.dimensional_analysis_gui")

    def _run_module(self, module_name: str):
        """Launch a calculator module from the project root."""
        env = os.environ.copy()
        ...
        try:
            subprocess.Popen([sys.executable, "-m", module_name], cwd=PROJECT_ROOT, env=env)
        except Exception as exc:
            messagebox.showerror("Launch Error", f"Could not launch '{module_name}'.\n\n{exc}")

if __name__ == "__main__":
    app = MainDashboard()
    app.start() # <--- MODIFIED: Call the start method instead of mainloop directly
